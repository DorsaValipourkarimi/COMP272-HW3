...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
         ...
         */
        if (node == null) { // Base case, where the node is null
            return null;
        }
        if (node != null) {// Otherwise when it's not null
            if (value < node.value) { // When the value is smaller, go to the left subtree
                node.leftChild = deleteElement(value, node.leftChild);
            } else if (value > node.value) { // When value is larger, go to the right subtree
                node.rightChild = deleteElement(value, node.rightChild);
            } else { // Otherwise, if the value is found
                // scenario: leaf node
                if (node.leftChild == null && node.rightChild == null) {
                    return null; // Just remove the node
                }
                // scenario 2: Node has only right child
                else if (node.leftChild == null) {
                    node = node.rightChild; // Rplace the node with its only child
                    // scenario 3: Node has only left child
                } else if (node.rightChild == null) {
                    node = node.leftChild; // Rplace the node with its only child
                    // scenario 4: Node with 2 children
                } else {
                    // Find the "Inorde Successor" node on its right subtree
                    Node successor = minValueNode(node.rightChild);
                    // Update node's value to successor value (a.k.a make the successor the leaf
                    // node)
                    node.value = successor.value;
                    // Delete the successor(duplicate value) from the right subtree
                    node.rightChild = deleteElement(successor.value, node.rightChild);
                }
            }
        }

        // If node becomes null, return it
        if (node == null) {
            return null;
        }
        // Update the height
        ...
        // Get the balance factor for rebalancing
        int bf = getBalanceFactor(node);

        // Performe rebalancing if necessary
        if (bf > 1) {
